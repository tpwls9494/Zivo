...
type MainTab = "search" | "profile" | "history";
    ...
    <div className="border rounded p-2 space-y-1">
      <div className="flex justify-between items-center">
        <span className="font-medium text-xs">
          {offer.carrier_iata} · {offer.carrier}
        </span>
        <span className="font-semibold text-sm">{formatKRW(offer.total_krw)}</span>
      </div>
      <div className="text-xs text-gray-600 flex gap-2">
        <span>
          {formatDate(offer.departure_at)} {formatTime(offer.departure_at)}
        </span>
        <span>→</span>
      ...
      {onBook && (
        <button
          className="w-full mt-1 bg-black text-white rounded py-1.5 text-xs font-medium"
          onClick={onBook}
        >
          예약하기
        </button>
...
function ComboCard({
  ...
  return (
    <div
      className={`border rounded p-2 space-y-2 ${bigSavings ? "border-green-400 bg-green-50" : ""}`}
    >
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          ...
          {combo.savings_krw > 0 && (
            <div
              className={`text-xs font-medium ${bigSavings ? "text-green-600" : "text-blue-600"}`}
            >
      ...
      {onBook && (
        <button
          className={`w-full mt-1 rounded py-1.5 text-xs font-medium text-white ${bigSavings ? "bg-green-600" : "bg-black"}`}
          onClick={onBook}
        >
          편도 조합 예약하기
        </button>
...
export default function App() {
  ...
  const [confirmCombo, setConfirmCombo] = useState<ComboOffer | null>(null);
  const showConfirm = confirmOffer !== null || confirmCombo !== null;
  ...
  function openConfirmOffer(offer: NormalizedOffer) {
    setConfirmCombo(null);
    setConfirmOffer(offer);
  }

  function openConfirmCombo(combo: ComboOffer) {
    setConfirmOffer(null);
    setConfirmCombo(combo);
  }
    ...
    <div className="p-4 space-y-3 text-sm min-w-[360px] max-w-[420px]">
      ...
      <div className="flex border-b text-xs">
        <button
          ...
          onClick={() => { setMainTab("search"); closeConfirm(); }}
        >
          검색
        </button>
        <button
          className={`px-3 py-2 ${mainTab === "profile" ? "border-b-2 border-black font-semibold" : "text-gray-500"}`}
          onClick={() => { setMainTab("profile"); closeConfirm(); }}
        >
          프로필
        </button>
        <button
          className={`px-3 py-2 ${mainTab === "history" ? "border-b-2 border-black font-semibold" : "text-gray-500"}`}
          onClick={() => { setMainTab("history"); closeConfirm(); }}
        >
          이력
        </button>
      </div>
      ...
      {mainTab === "history" && <BookingHistory />}
                  ...
                  {resultTab === "combo" && (
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                      ...
                      <div className="bg-yellow-50 border border-yellow-300 rounded p-2 text-xs text-yellow-800">
                        두 편이 별개 예약입니다. 앞 편 지연 시 뒷 편 항공사의 자동 보호가
                        적용되지 않습니다.
                      </div>
                      ...
                      {searchResult.combos.length === 0 ? (
                        ...
                        searchResult.combos.map((combo, i) => (
                          <ComboCard
                            key={`${combo.outbound.offer_id}-${combo.inbound.offer_id}-${i}`}
                            combo={combo}
                            onBook={() => openConfirmCombo(combo)}
                          />
                        ))
                      )}
                    </div>
